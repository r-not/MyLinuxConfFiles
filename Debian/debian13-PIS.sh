...
sudo nala update && sudo apt install -y vlc ffmpeg libavcodec-extra libdvdcss2 gstreamer1.0-plugins-bad gstreamer1.0-plugins-ugly audacious audacious-plugins handbrake-gtk kodi

echo "${GREEN}=== Installing Localsend ===${NC}"
...
echo "${GREEN}=== Tweaking Login Greeter ===${NC}"
...
echo "${GREEN}=== Installing Cinnamon Themes ===${NC}"
...
sudo nala install -y mint-y-icons
...
sudo nala install -y fonts-noto fonts-noto-color-emoji fonts-noto-extra fonts-noto-unhinted    
...
echo "${GREEN}=== Installing Browsers ===${NC}"
sudo nala remove --purge -y firefox-esr*
...
sudo nala update && sudo nala install -y firefox firefox-l10n-bn chromium
curl -fsS https://dl.brave.com/install.sh | sh

echo "${GREEN}=== Installing Power Management Tools ===${NC}"
sudo nala install -y tlp tlp-rdw
sudo systemctl enable tlp

echo "${GREEN}=== Installing Timeshift (Backup Tool) ===${NC}"
sudo nala install -y timeshift

echo "${GREEN}=== Enabling Flatpak ===${NC}"
sudo nala install -y flatpak gnome-software-plugin-flatpak
flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo

echo "${GREEN}=== Installing & Enabling Firewall (UFW) ===${NC}"
